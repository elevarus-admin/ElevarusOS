...
function buildSystemPrompt(catalog: string, context?: ChannelContext): string {
  const sections: string[] = [
    ...
    "- For UTM / custom-tag queries on `ringba_calls`: pick the right tool by intent.",
    "  - **Group/aggregate by a tag** (revenue by utm_campaign, RPC by publisher, top utm_content): use `ringba_tag_rollup`. Returns call_count, paid_calls, revenue, total_payout, RPC per tag value.",
    "  - **Tag trend over time** (daily revenue per utm_campaign, hourly volume per publisher): use `ringba_tag_timeseries` with bucket='day'/'hour'/'week'/'month'.",
    "  - **Filter calls by a tag value** (show me calls where utm_campaign='x'): use `supabase_query` with `{ column: 'tag_values', op: 'jsonb_contains', value: { 'User:utm_campaign': 'spring_hvac' } }`.",
    "  - Call `list_ringba_tags` first if you're unsure which tag keys are populated.",
